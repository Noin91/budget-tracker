...
{#snippet renameForm(action: string, id: number, name: string)}
	<form method="POST" {action} use:enhance class="flex items-center gap-1.5">
		...
		<input
			...
			name="name"
			value={name}
			class="w-full rounded-lg border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-800"
		/>
		<button
			type="submit"
...
<h1 class="mb-6 text-xl font-bold">Kategorien</h1>
	...
	{#each data.tree as group (group.id)}
		<Card title={gruppenLabel[group.type]}>
			<div class="space-y-3">
				{#each group.categories as category (category.id)}
					<div class="rounded-xl border border-slate-100 p-3 dark:border-slate-800">
						...
						{#if group.type === 'expense_fixed'}
							<div class="mt-2 ml-4 space-y-1.5 border-l border-slate-100 pl-3 dark:border-slate-800">
								{#each category.subcategories as sub (sub.id)}
									{@render renameForm('?/renameSubcategory', sub.id, sub.name)}
								{/each}
								<form method="POST" action="?/createSubcategory" use:enhance class="flex items-center gap-1.5">
									<input type="hidden" name="categoryId" value={category.id} />
									<input
										type="text"
										name="name"
										placeholder="Neue Unterkategorie"
										class="w-full rounded-lg border border-dashed border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-800"
									/>
									<button
										type="submit"
				...
				<form method="POST" action="?/createCategory" use:enhance class="flex items-center gap-1.5">
